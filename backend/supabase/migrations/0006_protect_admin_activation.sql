...
as $$
begin
  -- Admin accounts must remain active regardless of which database/API role
  -- performs the update. Administrative membership remains bootstrap-only.
  if new.role = 'admin' and not new.is_active then
